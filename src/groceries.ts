...
function createGroceries(): string[] {
  ...
  return ["Ketchup", "Mayonnase", "toast", "groundbeef", "eggs", "milk"]; // replace empty array with what you see is fit
}
...
function removeLastGroceryItem(groceries: string[]): string | undefined {
  // write your code here...
  let items = groceries.pop();
  return items; // replace empty string with what you see is fit
}
...
function getFirstThreeGroceryItems(groceries: string[]): string[] {
  let start: string[] = [groceries[0], groceries[1], groceries[2]];

  return start; // replace empty array with what you see is fit
}
